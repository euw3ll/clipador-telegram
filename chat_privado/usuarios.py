...
def get_nivel_usuario(telegram_id, nome=None):
    with sqlite3.connect(CAMINHO_BANCO) as conn:
        cursor = conn.cursor()
        cursor.execute("SELECT tipo FROM usuarios WHERE telegram_id = ?", (telegram_id,))
        row = cursor.fetchone()
...
def registrar_usuario(telegram_id, nome, tipo=1):
    with sqlite3.connect(CAMINHO_BANCO) as conn:
        cursor = conn.cursor()
        cursor.execute(
            "INSERT OR IGNORE INTO usuarios (telegram_id, nome, tipo) VALUES (?, ?, ?)",
...
def atualizar_usuario(telegram_id, nome=None, tipo=None):
    with sqlite3.connect(CAMINHO_BANCO) as conn:
        ...
        if nome and tipo is not None:
            cursor.execute("UPDATE usuarios SET nome = ?, tipo = ? WHERE telegram_id = ?", (nome, tipo, telegram_id))
        elif nome:
            cursor.execute("UPDATE usuarios SET nome = ? WHERE telegram_id = ?", (nome, telegram_id))
        elif tipo is not None:
            cursor.execute("UPDATE usuarios SET tipo = ? WHERE telegram_id = ?", (tipo, telegram_id))
        conn.commit()
...
def listar_usuarios():
    with sqlite3.connect(CAMINHO_BANCO) as conn:
        cursor = conn.cursor()
        cursor.execute("SELECT telegram_id, nome, tipo FROM usuarios")
